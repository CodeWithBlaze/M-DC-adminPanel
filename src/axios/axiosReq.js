...
API.interceptors.request.use((req) => {
  if (localStorage.getItem("dcuser")) {
    req.headers.Authorization = `Bearer ${
      JSON.parse(localStorage.getItem("dcuser")).token
    }`;
  }
  return req;
});
...
export const addClass = async (classBody) => {
  ...
  let res = await API.post(
    `/institute/class/${classBody.institute_id}`,
    classBody
  );
  return res;
};
...
export const getTutor = async () => {
  let res = await API.get("/d2c/teachers");
  return res;
};

export const updateTutorStatus = async (tutor) => {
  let res = await API.put("/d2c/teachers/verification", tutor);
  return res;
};
